...
MODEL_PATH = "/data/rstudio/embeddings/best"
...
ANALOGY_OUTPUT_PATH = "/home/alex/Projects/taxembed/2_validation/SkeletonKey/output/best"
PAIR_OUTPUT_PATH = "/home/alex/Projects/taxembed/2_validation/SkeletonKey/output/best"
...
################################################################
#           |                                                  #
# CLASS:    | BadAnalogyFile                                   #
#-----------|--------------------------------------------------#
# USE:      | This class inherits from Exception. It denotes:  #
#           |                                                  #
#           | 1. analogy files that are in the wrong format    #
#           | 2. analogy files where no tetrads (all 4 words)  #
#           |    appear in the embedding vocabulary            #
#           |                                                  #
################################################################
class BadAnalogyFile(Exception):
  
  # create an error for analogy files where no tetrads appear in the embedding analogies
  def __init__(self, message = "Bad Analogy File"):
...
# USE:      | This class inherits from Exception. It denotes:  #
#           |                                                  #
#           | 1. pair files that are in the wrong format       #
#           | 2. pair files where no tuples (both words)       #
#           |    appear in the embedding vocabulary            #
#           |                                                  #
################################################################
class BadPairFile(Exception):
  
  # create an error for analogy files where no tetrads appear in the embedding analogies
  def __init__(self, message = "Bad Pair File"):
    self.message = message
    super().__init__(self.message)

################################################################
#           |                                                  #
# FUNCTION: | parallel_analogy                                 #
#-----------|--------------------------------------------------#
# USE:      | provides parallel support for analogy evaluation #
#           |                                                  #
################################################################
def parallel_analogy(path, analogy_path):
  ...
  try:
    ...
    loaded_model = gensim.models.KeyedVectors.load(path)
    keyedflag = True
  
  try:
    if keyedflag == False:
      eval_res = store_analogy_eval(loaded_model.wv.evaluate_word_analogies(analogy_path))
    else:
      eval_res = store_analogy_eval(loaded_model.evaluate_word_analogies(analogy_path))
  except TypeError as e:
    del loaded_model
    raise BadAnalogyFile(analogy_path)
  else:
    # get the name information
    name_res = name_parse(path)
    # delete the loaded model
    del loaded_model
    # return results on success
    return [name_res, eval_res]
...
#-----------|--------------------------------------------------#
# USE:      | provides parallel support for pair    evaluation #
#           |                                                  #
################################################################
def parallel_pair(path, pair_path):
  ...
  try:
    if keyedflag == False:
      eval_res = store_pair_eval(loaded_model.wv.evaluate_word_pairs(pair_path))
    else:
      eval_res = store_pair_eval(loaded_model.evaluate_word_pairs(pair_path))
  except TypeError as e:
    del loaded_model
    raise BadPairFile(pair_path)
  else:
    # get the name information
    name_res = name_parse(path)
    # delete the loaded model
...
def perform_analogy_evals(model_path_list, analogy_path_list):
  ...
  for analogy_path in analogy_path_list:
    ...
    # create an empty list to store evaluation and name dictionaries
    info_eval_res = []
    
    # initialize multiprocessing.pool
    pool = mp.Pool(mp.cpu_count())
    
    # use pool apply for parallel processing
    try:
      info_eval_res = pool.starmap(parallel_analogy, [(path, analogy_path) for path in model_path_list])
    except BadAnalogyFile as e:
      print(e.message)
      print(analogy_path_list.index(e.message))
      continue
    else:
      for ev_obj in info_eval_res:
        name_info_dicts.append(ev_obj[0])
        analogy_eval_list.append(ev_obj[1])
    finally:
      pool.close()
...
def perform_pair_evals(model_path_list, pair_path_list):
  ...
  for pair_path in pair_path_list:
    ...
    # use pool apply for parallel processing
    try:
      info_eval_res = pool.starmap(parallel_pair, [(path, pair_path) for path in model_path_list])
    except BadPairFile as e:
      print(e.message)
      print(pair_path_list.index(e.message))
      continue
    else:
      for ev_obj in info_eval_res:
        name_info_dicts.append(ev_obj[0])
        pair_eval_list.append(ev_obj[1])
    finally:
      pool.close()
