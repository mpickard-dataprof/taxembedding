...
def word2vec2tensor(word2vec_model_path, tensor_filename, binary=False):
    ...
    with utils.open(outfiletsv, 'wb') as file_vector, utils.open(outfiletsvmeta, 'wb') as file_metadata:
        file_metadata.write((gensim.utils.to_utf8("word") + gensim.utils.to_utf8('\t') + gensim.utils.to_utf8("index") + gensim.utils.to_utf8('\n')))
        indexes = range(len(model.index2word))
        i = 0
        for word in model.index2word:
            file_metadata.write(gensim.utils.to_utf8(word) + gensim.utils.to_utf8('\t') + gensim.utils.to_utf8(str(indexes[i])) + gensim.utils.to_utf8('\n'))
