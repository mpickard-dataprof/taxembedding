...
get_page_urls <- function() {
  ...
  home_url <- "https://www.irs.gov/instructions"
  
  message("Scraping IRS Instruction URLS....")
...
extract_instructions <- function() {
  
  rds_dir <- "/data/rstudio/rds"
  rds_path <- path(rds_dir, "instructions.rds")
  
  message("-----Extracting IRS Instructions-----")
  ...
  if(file_exists(rds_path)) {
    ...
    instructions <- readRDS(rds_path)
    return(invisible(instructions))
  }
  
  instruction_urls <- get_page_urls()
  
  plan(multiprocess, workers = 10)
  text <- unlist(future_map(instruction_urls, scrape_page_html))
